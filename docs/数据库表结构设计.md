...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN ph.is_winner = TRUE THEN g.id END) as total_wins,
    ...
    CASE
        ...
        THEN ROUND(COUNT(DISTINCT CASE WHEN ph.is_winner = TRUE THEN g.id END) * 100.0 / COUNT(DISTINCT g.id), 2)
        ELSE 0
    END as win_rate
FROM players p
...
-- 玩家状态查询优化
CREATE INDEX idx_room_player_status ON players(room_id, player_status, is_spectator);
...
BEGIN
    INSERT INTO game_statistics (
        ...
        p_room_id, p_player_id, 1, IF(p_is_winner, 1, 0),
        p_game_score, p_game_score, p_game_score, p_game_score,
        NOW(), NOW()
    )
    ON DUPLICATE KEY UPDATE
        total_games = total_games + 1,
        total_wins = total_wins + IF(p_is_winner, 1, 0),
...
BEGIN
    ...
    spectator_count = (
        SELECT COUNT(*) FROM players
        WHERE room_id = NEW.room_id AND is_spectator = TRUE
    ),
    updated_at = NOW()
...
BEGIN
    ...
    SET current_players = (
        SELECT COUNT(*) FROM players
        WHERE room_id = OLD.room_id AND is_spectator = FALSE
    ),
    spectator_count = (
        SELECT COUNT(*) FROM players
        WHERE room_id = OLD.room_id AND is_spectator = TRUE
    ),
    updated_at = NOW()
